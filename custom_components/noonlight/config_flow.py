...
def _location_unique_id(
    environment: str, address: str, zip_code: str
) -> str:
    ...
    """
    return "_".join(
        [slugify(environment), slugify(address), slugify(zip_code)]
    )
...
class NoonlightConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_defaults(
        ...
        if user_input is not None:
            self._options = {
                CONF_DEFAULT_ENTRY_DELAY: int(
                    user_input[CONF_DEFAULT_ENTRY_DELAY]
                ),
    ...
    async def async_step_safety(
        ...
        return self.async_show_form(
            step_id="safety", data_schema=schema, errors=errors
        )
        ...
        if user_input is not None:
            return self.async_create_entry(
                ...
                data={
                    ...
                    CONF_DEDUPE_SECONDS: int(user_input[CONF_DEDUPE_SECONDS]),
                    CONF_SERVICES_GRANTED: user_input[CONF_SERVICES_GRANTED],
                    CONF_HEARTBEAT_MINUTES: int(
                        user_input[CONF_HEARTBEAT_MINUTES]
                    ),
                },
            )
