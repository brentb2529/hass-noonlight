...
class NoonlightCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    def _entry_delay_default(self) -> int:
        return self.entry.options.get(
            CONF_DEFAULT_ENTRY_DELAY, DEFAULT_ENTRY_DELAY
        )
    ...
    def _apply_refresh_interval(self, state: str) -> None:
        ...
        the heartbeat cadence while idle, and no idle polling at all when the
        heartbeat is disabled. Stay fast until the first heartbeat runs so
        startup detection isn't delayed."""
        interval = self._heartbeat_interval
    ...
    def _clear_health_issues(self) -> None:
        ...
        for key in (
            ...
            ir.async_delete_issue(
                self.hass, DOMAIN, f"{key}_{self.entry.entry_id}"
            )
    ...
    async def _poll_active_alarm(self) -> dict[str, Any]:
        ...
        await self._record_event(
            EVENT_STATUS_UPDATED, {"remote_status": remote_status}
        )
    ...
    async def async_test_dispatch(self) -> dict[str, Any]:
        ...
        try:
            ...
            raise
        await self._record_event(
            EVENT_TEST_DISPATCH, {"alarm_id": result.get("id")}
        )
        return result
    ...
    def _settle_callback(self, _now: Any) -> None:
        ...
        if self.data["state"] != STATE_CANCELED:
            return
        self._transition(
            STATE_IDLE, event=EVENT_CLEARED, alarm_id=None, services=[]
        )
    ...
    async def _record_event(
        self, event: str, detail: dict[str, Any]
    ) -> None:
