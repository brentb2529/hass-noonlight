...
def _has_issue(hass, key, entry_id) -> bool:
    return (
        ir.async_get(hass).async_get_issue(DOMAIN, f"{key}_{entry_id}")
        is not None
    )
...
async def test_heartbeat_disabled_when_zero(hass, setup_entry):
    ...
    assert coordinator._heartbeat_due() is False  # disabled, never due
    # While idle + disabled the coordinator does no automatic polling.
...
async def test_heartbeat_success_marks_healthy(hass, setup_entry):
    ...
    assert (
        hass.states.get(
            "binary_sensor.noonlight_main_api_reachable"
        ).state
        == "on"
    )
...
async def test_heartbeat_5xx_outage_is_unhealthy(hass, setup_entry):
    ...
    assert (
        hass.states.get("binary_sensor.noonlight_main_api_reachable").state
        == "off"
    )
