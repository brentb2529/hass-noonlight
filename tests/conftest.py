...
def _isolate_audit_log(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    ``hass.config.path(".storage", ...)`` which, in tests, resolves into the
    config dir and would dirty the working tree. Redirect it to the per-test
    ``tmp_path`` (an absolute path wins the ``hass.config.path`` join).
    """
    monkeypatch.setattr(
        "custom_components.noonlight.coordinator.AUDIT_FILE_TEMPLATE",
...
async def setup_entry(hass, config_entry: MockConfigEntry):
    ...
    with respx.mock:
        respx.get(
            url__regex=r".*/dispatch/v1/alarms/.*/status"
