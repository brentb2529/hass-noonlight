...
from custom_components.noonlight.services import async_setup_services
...
@respx.mock
async def test_blank_account_resolves_single_site(hass, setup_entry):
...
async def test_test_dispatch_hits_sandbox(hass, setup_entry):
    ...
    await hass.services.async_call(
        DOMAIN, SVC_TEST_DISPATCH, {}, blocking=True
    )
    await hass.async_block_till_done()
...
async def test_multiple_accounts_require_account_arg(hass, setup_entry):
    ...
    with pytest.raises(ServiceValidationError):
        await hass.services.async_call(
            DOMAIN, SVC_DISPATCH_POLICE, {"entry_delay_seconds": 0},
            blocking=True,
        )
...
async def test_setup_services_is_idempotent(hass, setup_entry):
...
@respx.mock
async def test_ambiguous_account_title_raises(hass, setup_entry):
